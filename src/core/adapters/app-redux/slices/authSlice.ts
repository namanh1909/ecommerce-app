...
const registerRequest: Reducer<PayloadAction<any>> = (state, { payload }: any) => {
    state.isProcessing = true;
};

const registerSuccess: Reducer<PayloadAction<{ token: string; user: any; refreshToken: string }>> = (state, { payload }) => {
    state.isProcessing = false;
    state.token = payload.token;
    state.refreshToken = payload.refreshToken;
    state.user = payload.user;
