...
import { AuthSelectors, authActions } from "../slices/authSlice";
...
function* handleLoginRequest({ payload }: PayloadAction<{ email: string; password: string }>): Generator {
    try {
...
function* handleRegisterRequest({ payload }: PayloadAction<{ email: string, password: string, name: string, phone: string }>): Generator {
    try {
        console.log('cendetial', payload)
        ...
        yield put(authActions.registerSuccess({ user: signInResp?.user, token: signInResp?.token, refreshToken: signInResp?.refreshToken }));
    } catch (ex: any) {
...
export function* signOut() {
    try {
        const refreshToken: string = yield call(helper.getDataFromAsyncStorage, constants.ASYNC_STORAGE_KEYS.REFRESH_TOKEN);
